...
for i=1:length(elementos_triangulares)-1
    x_max=max(temperaturas(elementos_triangulares(i,:),1));
    x_min=min(temperaturas(elementos_triangulares(i,:),1));
    y_max=max(temperaturas(elementos_triangulares(i,:),2));
    y_min=min(temperaturas(elementos_triangulares(i,:),2));
    %Si esta dentro del elemento entonces es el elemento para interpolar
    if(punto_calculo(1)<x_max && punto_calculo(1)>x_min && punto_calculo(2) <y_max && punto_calculo(2) >y_min)
        elemento_t = i;
    else
